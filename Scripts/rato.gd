...
@export_range(0.0, 1.0, 0.01) var chance_coracao := 0.12
...
func die() -> void:
	if morto:
		return
	morto = true
	_tentar_dropar_coracao()
	queue_free()

func _tentar_dropar_coracao() -> void:
	if randf() > chance_coracao:
		return
	var drop := HEART_SCENE.instantiate()
	var mundo := _world_parent()
	drop.position = global_position
	mundo.add_child(drop)

func _world_parent() -> Node:
	var node: Node = self
	while node.get_parent() != null:
		node = node.get_parent()
		if node.get_node_or_null("Player") != null and node.get_node_or_null("Enemies") != null:
			return node
