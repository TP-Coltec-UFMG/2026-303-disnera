...
@export var contraste: CheckButton
...
@export var comecar_button: Button
...
func _ready() -> void:
	...
	comecar_button.pressed.connect(_comecar_jogo)
	
	contraste.button_pressed = (
	AcessibilidadeManager.esta_em_alto_contraste()
)
	contraste.toggled.connect(_alternar_alto_contraste)

func _alternar_alto_contraste(ativado: bool) -> void:
	AcessibilidadeManager.definir_alto_contraste(ativado)
